...
func initializeDatabase() {
	db, err := sql.Open("postgres", connStr); if err != nil { log.Fatal("Falha ao conectar ao DB para inicialização: ", err) }; defer db.Close()
	...
	var userCount int; db.QueryRow("SELECT COUNT(*) FROM users WHERE email = $1", "edgard@peoplepulse.com.br").Scan(&userCount)
	if userCount == 0 {
		log.Println("Usuário de teste não encontrado, criando..."); password := []byte("12345"); hashedPassword, _ := bcrypt.GenerateFromPassword(password, bcrypt.DefaultCost)
		var insertedUserID int; db.QueryRow("INSERT INTO users (name, email, password_hash, position, role) VALUES ($1, $2, $3, $4, $5) RETURNING id", "Edgard Masso", "edgard@peoplepulse.com.br", string(hashedPassword), "Diretor", "diretoria").Scan(&insertedUserID)
	...
	} else { log.Println("Banco de dados já inicializado.") }
}
...
func main() {
	initializeDatabase(); router := gin.Default(); router.Use(cors.Default()); router.POST("/login", Login); router.GET("/kpis", GetKPIs); router.Run(":8080")
}